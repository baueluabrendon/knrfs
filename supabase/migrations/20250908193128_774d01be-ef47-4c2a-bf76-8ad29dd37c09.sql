-- Create comprehensive dashboard view with corrected enum values
...
daily_metrics AS (
  SELECT 
    ...
    -- Add statutory body count (assuming some loans might have this type)
    0 AS statutory_body_count,
    ...
    COUNT(a.application_id) FILTER (WHERE a.uploaded_at::date <= ds.analysis_date AND a.status = 'declined') AS declined_applications_count,
    ...
    -- Average loan duration (for settled loans as of this date) - convert to numeric days
    COALESCE(AVG(EXTRACT(days FROM (l.settled_date - l.disbursement_date))), 0) AS avg_loan_duration_days,
